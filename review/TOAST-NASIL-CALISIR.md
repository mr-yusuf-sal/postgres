...
içindeki tek `if`'i gösteriyor: tuple çok büyükse toaster çağrılıyor.
[UPDATE-NASIL-CALISIR.md](UPDATE-NASIL-CALISIR.md) ise `heap_update`'in TOAST
gerektiğinde buffer kilidini bırakmasını anlatıyor. Bu not o `if`'in arkasına
giriyor. TOAST = **T**he **O**versized-**A**ttribute **S**torage **T**echnique.
...
```
  INSERT ... (metin: 500 KB)
       ...
       ▼
  ┌────────────────────────────────────────────────────────────┐
  │ heap_toast_insert_or_update — dört tur; her turda:         │
  │    "kalan en BÜYÜK varlena kolonu bul, ona müdahale et"    │
  │  Tur 1: EXTENDED kolonları SIKIŞTIR (pglz / lz4)           │
  │  Tur 2: EXTENDED + EXTERNAL kolonları DIŞARI TAŞI          │
  │  Tur 3: MAIN kolonları sıkıştır                            │
  │  Tur 4: MAIN kolonları dışarı taşı (son çare)              │
  └────────────────────────┬───────────────────────────────────┘
                           ▼  dışarı taşınan her değer için
  ┌────────────────────────────────────────────────────────────┐
  │ toast_save_datum: 1996 baytlık chunk'lara böl              │
  │   pg_toast.pg_toast_16384                                  │
  │    chunk_id | chunk_seq | chunk_data     + UNIQUE btree    │
  │     24601   |  0, 1, 2  | [<=1996 bayt]    (chunk_id, seq) │
  └────────────────────────┬───────────────────────────────────┘
...
4. **Dışarı taşınan değer parçalanır**: `(chunk_id, chunk_seq, chunk_data)` +
   üzerinde UNIQUE btree. Chunk 1996 bayt — sabit.
...
([../src/backend/catalog/toasting.c:432-433](../src/backend/catalog/toasting.c#L432-L433));
heap'in cevabı üç adımlı
([../src/backend/access/heap/heapam_handler.c:2040-2047](../src/backend/access/heap/heapam_handler.c#L2040-L2047)):
toast'lanabilir kolon yoksa hayır; sınırsız uzunlukta bir kolon (`text`, `bytea`,
tipmod'suz `varchar`) varsa evet; hepsi sınırlıysa en kötü durum uzunluğu
`TOAST_TUPLE_THRESHOLD` ile karşılaştırılır — sadece `varchar(10)` kolonlarından
oluşan bir tablo TOAST tablosu almaz. Partitioned tablolar, shared kataloglar ve
initdb sonrası kataloglar da elenir
([../src/backend/catalog/toasting.c:407-430](../src/backend/catalog/toasting.c#L407-L430)).
...
`plain` ile `external` farkı önemli: `plain` **hiç TOAST'lanmaz** — değer
sığmazsa INSERT hata verir. `external` dışarı taşınır, sadece sıkıştırılmaz.

**`ALTER TABLE ... ALTER COLUMN ... SET STORAGE` sadece bir katalog
güncellemesidir** — `GetAttributeStorage` harfi çözer, `attstorage` alanı
`CatalogTupleUpdate` ile yazılır, mevcut satırlara dokunulmaz
([../src/backend/commands/tablecmds.c:9345-9347](../src/backend/commands/tablecmds.c#L9345-L9347)).
Yeni değer bundan sonraki INSERT/UPDATE'lerde geçerlidir; eski satırlar için
`VACUUM FULL` ya da dummy bir `UPDATE` gerekir. Toast'lanamayan bir tipe `plain`
dışında strateji verilemez
([../src/backend/commands/tablecmds.c:22822-22846](../src/backend/commands/tablecmds.c#L22822-L22846)).
...
([satır 3839-3842](../src/backend/access/heap/heapam.c#L3839-L3842)); eski
tuple'ın dışarıda değeri varsa da çağrılır, çünkü o değerin silinip
silinmeyeceğine karar verilmelidir (bölüm 9). Her iki dalda da TOAST tablolarının
kendisi elenir — özyineleme felaket olurdu
([satır 2260-2262](../src/backend/access/heap/heapam.c#L2260-L2262)).
...
```
— [../src/backend/access/heap/heaptoast.c:162-167](../src/backend/access/heap/heaptoast.c#L162-L167)
...
([satır 187](../src/backend/access/table/toast_helper.c#L187)): 24 bayttan küçük
bir değeri dışarı taşımak boşuna, yerine yazılacak işaretçi zaten o kadar yer
tutuyor. `for_compression` bayrağı ayrıca `TOASTCOL_INCOMPRESSIBLE` kolonları
eler ([satır 191-192](../src/backend/access/table/toast_helper.c#L191-L192)); üç
filtre daha var — zaten dışarıda olan, zaten sıkışık ve `check_main`'e uymayan
`attstorage`'lar atlanır
([satır 198-209](../src/backend/access/table/toast_helper.c#L198-L209)).
...
([satır 215-217](../src/backend/access/heap/heaptoast.c#L215-L217)); gerekçe
yorumda: yoksa "bir uzun ve birkaç kısa kolon" senaryosunda diğerlerini boşuna
sıkıştırırsın ([satır 208-213](../src/backend/access/heap/heaptoast.c#L208-L213)).
...
([../src/include/access/heaptoast.h:52-61](../src/include/access/heaptoast.h#L52-L61)).

Her turda `if (biggest_attno < 0) break;` var; iş kalmadığında döngü sessizce
biter. Bir kolon değiştiyse `TOAST_NEEDS_CHANGE` yanar ve tuple sıfırdan kurulur
([satır 277](../src/backend/access/heap/heaptoast.c#L277)); `natts` ve `t_hoff`
yeniden hesaplanır, çünkü `ALTER TABLE ADD COLUMN` sonrası eski tuple'ların
natts'ı farklı olabilir
([satır 288-293](../src/backend/access/heap/heaptoast.c#L288-L293)).
...
GUC tanımı
[../src/backend/utils/misc/guc_parameters.dat:784-789](../src/backend/utils/misc/guc_parameters.dat#L784-L789);
...
**İki farklı "başarısız" var.** Kütüphane pes eder: pglz girdi çok kısa ya da çok
uzunsa hiç denemez
...
([satır 166-170](../src/backend/access/common/toast_compression.c#L166-L170)).
Ya da PostgreSQL kazancı yeterli bulmaz — kütüphane "başardım" dese bile ikinci
...
([satır 81-90](../src/backend/access/common/toast_internals.c#L81-L90)). `NULL`
dönerse kolon `TOASTCOL_INCOMPRESSIBLE` damgası yer ve bir daha denenmez
([../src/backend/access/table/toast_helper.c:247-248](../src/backend/access/table/toast_helper.c#L247-L248)).
JPEG, ZIP, şifreli veri bu dala düşer — sıkıştırma CPU'su boşa gider; bu tür
kolonlar için `EXTERNAL` doğru seçimdir.
...
([../src/include/access/toast_internals.h:39-46](../src/include/access/toast_internals.h#L39-L46)).
Bu yüzden `default_toast_compression`'ı değiştirmek eski satırları etkilemez;
aynı kolonda pglz ve lz4 değerler yan yana durabilir.
...
[satır 161-187](../src/backend/access/common/toast_internals.c#L161-L187));
sıkışık olan en öğreticisi:
...
hem açılır. Value ID, TOAST index'i üzerinden kullanılmayan bir OID olarak
seçilir ([satır 217-220](../src/backend/access/common/toast_internals.c#L217-L220));
tablo yeniden yazılırken (`CLUSTER`, `VACUUM FULL`) eski ID korunur ve aynı
değere işaret eden birden fazla satır versiyonu varsa veri **ikinci kez
yazılmaz** ([satır 255-260](../src/backend/access/common/toast_internals.c#L255-L260)).
...
```
— [../src/backend/access/common/toast_internals.c:283-314](../src/backend/access/common/toast_internals.c#L283-L314)
(yorumlar ve `SET_VARSIZE` çıkarıldı)
...
8 KB blokta: `2032 − 24 − 4 − 4 − 4 = **1996**`. `initdb` gerektirir, çalışırken
değiştirilemez ([satır 78](../src/include/access/heaptoast.h#L78)).
...
([satır 327-336](../src/backend/access/common/toast_internals.c#L327-L336));
birden fazla index olabilmesinin sebebi `REINDEX CONCURRENTLY`'dir, okuma tarafı
`indisvalid` olanı seçer
([satır 576-586](../src/backend/access/common/toast_internals.c#L576-L586)).
Sonunda `palloc(TOAST_POINTER_SIZE)` + `SET_VARTAG_EXTERNAL(result,
VARTAG_ONDISK)` ile işaretçi kurulur
([satır 362-364](../src/backend/access/common/toast_internals.c#L362-L364)).
`TOAST_POINTER_SIZE` = 2 + 16 = **18 bayt**
([../src/include/access/detoast.h:31](../src/include/access/detoast.h#L31)):
1 GB'lık bir `text` de, 3 KB'lık bir `text` de ana tabloda 18 bayt yer kaplar.

## 6.2 TOAST tablosunun şeması

`create_toast_table`
([../src/backend/catalog/toasting.c:128](../src/backend/catalog/toasting.c#L128))
üç kolonluk (`chunk_id` OID, `chunk_seq` int4, `chunk_data` bytea) bir tablo
kurar ([satır 205-217](../src/backend/catalog/toasting.c#L205-L217)). Ad **ana
tablonun OID'inden** türetilir — `pg_toast_<relOid>`, adından değil, o yüzden
`ALTER TABLE ... RENAME` TOAST adını değiştirmez
([satır 199-202](../src/backend/catalog/toasting.c#L199-L202)).
...
([satır 220-222](../src/backend/catalog/toasting.c#L220-L222)). Üç kolonun da
`attstorage`'ı `TYPSTORAGE_PLAIN` yapılır
([satır 224-226](../src/backend/catalog/toasting.c#L224-L226)) ve sıkıştırma
kapatılır ([satır 228-231](../src/backend/catalog/toasting.c#L228-L231)).

Kalıcı tablolar `pg_toast` şemasına (OID 99,
[../src/include/catalog/pg_namespace.dat:18](../src/include/catalog/pg_namespace.dat#L18)),
geçici tablolar backend'e özel geçici toast şemasına gider
([../src/backend/catalog/toasting.c:243-246](../src/backend/catalog/toasting.c#L243-L246)).
Index `(chunk_id, chunk_seq)` üzerinde UNIQUE btree'dir
([satır 328-335](../src/backend/catalog/toasting.c#L328-L335)); yorum gerekçeyi
veriyor: tek kolon da işe yarardı ama **dilim erişimi için ikinci kolon şart**,
UNIQUE olması da OID çakışmalarına karşı sigorta
([satır 286-292](../src/backend/catalog/toasting.c#L286-L292)). Bağlantı
`reltoastrelid` ile kurulur ([satır 351](../src/backend/catalog/toasting.c#L351)),
`DEPENDENCY_INTERNAL` ile ana tabloya bağlanır
([satır 393](../src/backend/catalog/toasting.c#L393)).
...
kısmıdır: kısa `text` değerleri 4 yerine 1 baytlık başlık kullanır ve hizalama
dolgusu yemez. Struct tanımları
[../src/include/varatt.h:126-154](../src/include/varatt.h#L126-L154).
...
`va_extinfo` iki bilgiyi tek `uint32`'ye sıkıştırır: alt 30 bit dışarıdaki gerçek
boyut, üst 2 bit sıkıştırma yöntemi
...
[../src/include/varatt.h:84-90](../src/include/varatt.h#L84-L90)):
`VARTAG_ONDISK = 18` bu notun konusu, veri TOAST tablosunda; `VARTAG_INDIRECT`
veriyi bellekte başka bir yerde gösterir; `VARTAG_EXPANDED_RO/RW` genişletilmiş
nesnedir (PL/pgSQL array'leri gibi) — hesaplama için uygun, depolama için uygun
olmayan bir format. Diskteki tuple'larda sadece `VARTAG_ONDISK` bulunur, ama
`detoast_attr` üçünü de karşılamak zorundadır.
...
beş dallı bir `if` zinciridir. İlk dal en sık kullanılanı: `toast_fetch_datum`
ile chunk'lar toplanır, sonuç hâlâ `VARATT_IS_COMPRESSED` ise
`toast_decompress_datum` ile açılır
([satır 118-132](../src/backend/access/common/detoast.c#L118-L132)). Kalan
dallar: indirect pointer (dereference + özyineleme), expanded object
(düzleştir), inline sıkışık (aç), kısa başlıklı (4 baytlığa çevir)
([satır 133-188](../src/backend/access/common/detoast.c#L133-L188)). Sonuç **her
zaman** düz, 4 baytlık başlıklı bir varlena'dır — bu yüzden `PG_GETARG_TEXT_PP`
yerine `PG_GETARG_TEXT_P` kullanan fonksiyonlar her çağrıda tam kopya öder.
Kardeşi `detoast_external_attr`
([satır 45](../src/backend/access/common/detoast.c#L45)) getirir ama
**sıkıştırmayı açmaz**; bölüm 4.1'deki yol bunu kullanır.
...
açsın ([satır 356-363](../src/backend/access/common/detoast.c#L356-L363)). Heap
...
([satır 663-683](../src/backend/access/heap/heaptoast.c#L663-L683)).
**Dilim okuma gerçekten sadece gereken chunk'ları okur.** Okurken dört bozulma
kontrolü var: chunk numarası beklenen mi
([satır 737-742](../src/backend/access/heap/heaptoast.c#L737-L742)), aralıkta mı
...
([../src/backend/access/common/toast_internals.c:643-646](../src/backend/access/common/toast_internals.c#L643-L646)).
Kural yorumda: **detoast, işaretçiyi getiren transaction ile aynı transaction'da
olmalı** ([satır 625-626](../src/backend/access/common/toast_internals.c#L625-L626)).
Bir prosedür toast'lı değeri değişkene alıp `COMMIT` edip sonra kullanmaya
kalkarsa chunk'lar aradan silinmiş olabilir.

## 8.2 `substr()` neden dilim okuyabiliyor?

Zincir: `substr` → `text_substring`
([../src/backend/utils/adt/varlena.c:586](../src/backend/utils/adt/varlena.c#L586))
→ `DatumGetTextPSlice` ([../src/include/fmgr.h:305](../src/include/fmgr.h#L305))
→ `pg_detoast_datum_slice`
([../src/backend/utils/fmgr/fmgr.c:1823](../src/backend/utils/fmgr/fmgr.c#L1823))
→ `detoast_attr_slice`
([../src/backend/access/common/detoast.c:205](../src/backend/access/common/detoast.c#L205)).
Oradaki *"fast path for non-compressed external datums"* dalı doğrudan
`toast_fetch_datum_slice` çağırır
([satır 232-234](../src/backend/access/common/detoast.c#L232-L234)).
...
([satır 254-256](../src/backend/access/common/detoast.c#L254-L256),
[../src/common/pg_lzcompress.c:857](../src/common/pg_lzcompress.c#L857)) ama
**LZ4 için mümkün değil** — kod yorumu açıkça söylüyor: uygun bir API çağrısı
olmadığı için tüm değer getirilir
([satır 249-252](../src/backend/access/common/detoast.c#L249-L252)). Her
hâlükârda sadece **prefix** hızlandırılabilir; ortadan dilim istenirse oraya
kadarı açılmak zorundadır.
...
Sıkıştırma zaten işe yaramaz (bölüm 5), `EXTERNAL` ile değer düz yazıldığı için
`substr()` hızlı yolu kullanır. Bedeli: sıkıştırılabilir veride `EXTENDED` yer
kazandırırdı. Tersi de doğru — uzun düz metin (log, JSON, HTML) tutan ve genelde
bütün okunan kolonlarda `EXTENDED` bırakılmalı.
...
```
— [../src/backend/access/table/toast_helper.c:76-97](../src/backend/access/table/toast_helper.c#L76-L97)
...
- **İşaretçiler aynıysa**: kolon `TOASTCOL_IGNORE` alır, yeni tuple aynı 18
  ...
  `bytea` kolonu olan satırda başka bir kolonu güncellemek o 1 GB'ı yeniden
  yazmaz.
- **Farklıysa**: eski chunk'lar `toast_tuple_cleanup` içinde silinir
  ([satır 299-310](../src/backend/access/table/toast_helper.c#L299-L310)).

`varatt_external`'ın dolgusuz olmak zorunda olmasının sebebi bu `memcmp`'dir
([../src/include/varatt.h:24-30](../src/include/varatt.h#L24-L30)). Silme
fiziksel değil — `simple_heap_delete` sadece `xmax` yazar, yer VACUUM'a kadar
geri gelmez. `heap_update`'in TOAST için buffer kilidini bırakması
[UPDATE-NASIL-CALISIR.md](UPDATE-NASIL-CALISIR.md) bölüm 4.5'te.
...
Speculative insertion iptalinde aynı yol `heap_abort_speculative` ile işletilir
([../src/backend/access/heap/heapam.c:6512](../src/backend/access/heap/heapam.c#L6512)).
...
([../src/backend/commands/vacuum.c:2284-2287](../src/backend/commands/vacuum.c#L2284-L2287)) —
...
([satır 2355-2367](../src/backend/commands/vacuum.c#L2355-L2367)).
`VACUUM (PROCESS_TOAST false) tablom` bu adımı kapatır
([satır 267-268](../src/backend/commands/vacuum.c#L267-L268)).
...
([../src/backend/postmaster/autovacuum.c:2133-2137](../src/backend/postmaster/autovacuum.c#L2133-L2137)).
Sebep hemen üstteki yorumda: *"we don't automatically vacuum toast tables along
the parent table"*
([satır 2098-2102](../src/backend/postmaster/autovacuum.c#L2098-L2102)).
...
([satır 2848-2857](../src/backend/postmaster/autovacuum.c#L2848-L2857)):
`ALTER TABLE tablom SET (toast.autovacuum_vacuum_scale_factor = 0.05);`

**ANALYZE hiç çalışmaz** — TOAST tablosu istatistik tutmaz
([satır 3314-3315](../src/backend/postmaster/autovacuum.c#L3314-L3315)). Gerekçe
...
([../src/backend/utils/adt/dbsize.c:457-458](../src/backend/utils/adt/dbsize.c#L457-L458)).
`calculate_toast_table_size` TOAST heap'ini **ve index'ini** toplar
([satır 396-431](../src/backend/utils/adt/dbsize.c#L396-L431)) — yani
`pg_total_relation_size − pg_indexes_size` yaparsan TOAST index'i içeride kalır.
...
([../src/include/access/toast_compression.h:49-53](../src/include/access/toast_compression.h#L49-L53)).
`pg_column_size` → `toast_datum_size`
([../src/backend/utils/adt/varlena.c:4186](../src/backend/utils/adt/varlena.c#L4186));
dış değer için `va_extinfo`'daki boyutu döner ve **18 baytlık işaretçiyi
saymaz** ([../src/backend/access/common/detoast.c:608-611](../src/backend/access/common/detoast.c#L608-L611)).
`pg_column_compression` yöntemi değerin kendisinden okur, kolon tanımından değil
([../src/backend/utils/adt/varlena.c:4234-4237](../src/backend/utils/adt/varlena.c#L4234-L4237));
`NULL` üç şey demek olabilir: değer sıkıştırılmamış, varlena değil, ya da
sıkıştırılmadan dışarı yazılmış (`EXTERNAL`). `pg_column_toast_chunk_id` `NULL`
değilse değer diskte dışarıdadır
([satır 4260](../src/backend/utils/adt/varlena.c#L4260)). TOAST I/O kolonları
`reltoastrelid` üzerinden LEFT JOIN ile gelir
([../src/backend/catalog/system_views.sql:826-833](../src/backend/catalog/system_views.sql#L826-L833)).
...
```
 ...
 │ toast_tuple_init → NULL/PLAIN/fixed kolonları ele                   │
 │                  → UPDATE'te memcmp: aynı işaretçi = DOKUNMA        │
 ...
 │   TUR 2 EXTENDED/EXTERNAL → dışarı at                               │
 │   TUR 3 MAIN → sıkıştır                                             │
 ...
 [toast_internals.c:46]  │ chunk_size = 1996, chunk_id = yeni OID
   ...
   kazanç ≤ 2 bayt       ▼
    → INCOMPRESSIBLE   pg_toast.pg_toast_<oid>
                       (chunk_id, chunk_seq, chunk_data)
                       UNIQUE btree (chunk_id, chunk_seq)
                         │
                         ▼
